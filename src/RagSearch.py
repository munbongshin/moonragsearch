...
class RAGChatApp:
    ...
    def load_groq(self):
        ...
        if not api_key:
            raise ValueError("GROQ_API_KEY가 환경 변수에 설정되지 않았습니다.")
        ...
        if models:
            self.lm_llm.model_name = models  # 첫 번째 모델을 기본값으로 사용
            self.set_llm_model(self.groq)
        else:
            st.error("사용 가능한 Groq 모델이 없습니다.")
            return None  # 모델이 없으면 None 반환
        # Groq 클라이언트 초기화
        try:
            groq_llm = ChatGroq(
                groq_api_key=api_key,
                model_name=models,  # None이 아닌 실제 모델 이름 사용
                temperature=0,
            )
            return groq_llm
        except Exception as e:
